...
namespace Graph{
	...
	const std::list<size_t>&directly_connected_to(const size_t number)const;
	const std::list<size_t> connected_to(const size_t number)const;
    private:
	std::vector<std::list<size_t>> f_data;
    };
};
#endif
