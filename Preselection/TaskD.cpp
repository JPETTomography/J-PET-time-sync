...
void TaskD::init(const JPetTaskInterface::Options& opts){
	...
	for(auto & layer : getParamBank().getLayers()){
		for (int thr=1;thr<=4;thr++){
			...
			getStatistics().createHistogram( new TH2F(histo_name, histo_titile, n_slots_in_layer, 0.5, n_slots_in_layer+0.5,120, -20., 20.) );
		}
	}
}
...
void TaskD::fillHistosForHit(const JPetHit & hit){
	...
	for(auto & thr_time_pair : lead_times_A){
		...
		if( lead_times_B.count(thr) > 0 ){
			...
			getStatistics().getHisto2D(Form("TimeDiffVsID_layer_%d_thr_%d", layer_number, thr)).Fill( slot_number,timeDiffAB);
		}
	}
}
