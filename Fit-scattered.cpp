...
int main(int argc, char **argv) {
	...
	hist<double> left,right,hleft,hright,chisq;
	for(size_t slot=1;slot<=Nei[i]->layerSize(layer);slot++){
	    ...
	    if(item.valid()){
		hleft<<point<value<double>>(double(slot),item.hleft);
		hright<<point<value<double>>(double(slot),item.hright);
	...
	";deltaID="+to_string(neighbour_delta_id[i])+"'";
	Plot<double>().Hist(hleft,"Height Left").Hist(hright,"Height Right")
	<<"set key on"<<title;
	Plot<double>().Hist(left,"Position Left").Hist(right,"Position Right")
	<<"set key on"<<title;
	...
	for(size_t i=0;i<2;i++){
		...
		if(res.valid()){
		    hleft<<point<value<double>>(slot,res.hleft);
		    hright<<point<value<double>>(slot,res.hright);
